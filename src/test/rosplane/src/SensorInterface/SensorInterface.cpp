...
namespace rosplane {
    ...
    void SensorInterface::runActionInterface() {
        ...
        ops.template init<rosplan_dispatch_msgs::ActionDispatch>(adt, 1000, boost::bind(&rosplane::SensorInterface::dispatchCallback, this, _1));
    ...
    /* run action interface */
    void SensorInterface::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) {
        ...
        if (action_success) {
            ...
            ros::NodeHandle nh;
            // TODO: 自适应服务名（根据action_dispatch修改/后面的部分）
            ros::ServiceClient _plan_dispatch_cancel_client = nh.serviceClient<std_srvs::Empty>("/rosplan_plan_dispatcher/cancel_dispatch");
