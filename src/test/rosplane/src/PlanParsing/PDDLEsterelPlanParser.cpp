#include "PlanParsing/PDDLEsterelPlanParser.h"
...
namespace rosplane {
    ...
    /**
     * plan parsing service method (3)
     * loads plan from string
     */
    bool PDDLEsterelPlanParser::parsePlanFromString(rosplane::ParsingStringEsterel::Request& req, rosplane::ParsingStringEsterel::Response& res) {
        ROS_INFO("KCL: (%s) Parsing plan from string.", ros::this_node::getName().c_str());
...
int main(int argc, char** argv) {
    ...
    rosplane::PDDLEsterelPlanParser pp(nh);

    // subscribe to planner output
    std::string planTopic = "planner_output";
    nh.getParam("planner_topic", planTopic);
    ros::Subscriber plan_sub = nh.subscribe(planTopic, 1, &rosplane::PlanParser::plannerCallback, dynamic_cast<rosplane::PlanParser*>(&pp));

    // start the plan parsing services
    ros::ServiceServer service1 = nh.advertiseService("parse_plan", &rosplane::PlanParser::parsePlan, dynamic_cast<rosplane::PlanParser*>(&pp));
    ros::ServiceServer service2 = nh.advertiseService("parse_plan_from_file", &rosplane::PlanParser::parsePlanFromFile, dynamic_cast<rosplane::PlanParser*>(&pp));
    ros::ServiceServer service3 = nh.advertiseService("parse_plan_from_string", &rosplane::PDDLEsterelPlanParser::parsePlanFromString, &pp);
