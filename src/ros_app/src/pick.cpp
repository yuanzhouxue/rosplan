...
namespace ros_app {
    ...
    /* constructor */
    pick_ActionInterface::pick_ActionInterface() : _nh("~"), play_m_as("/play_motion"), pick_as("/pickup_pose"), tf_l(tfBuffer) {
        ...
        service = _nh.advertiseService("my_pick_service", &pick_ActionInterface::callback, this);
    }

    bool pick_ActionInterface::callback(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res) {
    ...
    string pick_ActionInterface::stripLeadingSlash(const string& s) {
        if (s[0] == '/') return std::move(s.substr(1));
        return std::move(s.substr());
    }

    void pick_ActionInterface::prepareRobot() {
    ...
    void pick_ActionInterface::lowerHead() {
    ...
    void pick_ActionInterface::liftTorso() {
    ...
    bool pick_ActionInterface::concreteCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) {
        // if (!robot_prepared) {
        prepareRobot();
        // robot_prepared = true;
    // }
        ...
        ros::ServiceClient get_aruco_state = _nh.serviceClient<gazebo_msgs::GetModelState>("/gazebo/get_model_state");
...
int main(int argc, char** argv) {
    ...
    ros_app::pick_ActionInterface rpti;
