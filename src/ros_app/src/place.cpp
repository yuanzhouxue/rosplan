...
namespace ros_app {
    ...
    /* constructor */
    place_ActionInterface::place_ActionInterface() : _nh("~"), play_m_as("/play_motion"), place_as("/place_pose"), tf_l(tfBuffer) {
        node_name = ros::this_node::getName();
        clear_costmaps_client_ = _nh.serviceClient<std_srvs::Empty>("/move_base/clear_costmaps");
        update_knowledge_client_ = _nh.serviceClient<KnowledgeUpdateService>("/rosplan_knowledge_base/update");
        update_knowledge_array_client_ = _nh.serviceClient<KnowledgeUpdateServiceArray>("/rosplan_knowledge_base/update_array");
        query_knowledge_client_ = _nh.serviceClient<GetInstanceService>("/rosplan_knowledge_base/state/instances");
        ...
        torso_cmd = _nh.advertise<trajectory_msgs::JointTrajectory>("/torso_controller/command", 1);
        head_cmd = _nh.advertise<trajectory_msgs::JointTrajectory>("/head_controller/command", 1);
        gripper_cmd = _nh.advertise<trajectory_msgs::JointTrajectory>("/gripper_controller/command", 1);
        detected_pos_pub = _nh.advertise<geometry_msgs::PoseStamped>("/detected_aruco_pose", 1, true);
        ...
        service = _nh.advertiseService("my_place_service", &place_ActionInterface::callback, this);
    }

    bool place_ActionInterface::callback(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res) {
    ...
    string place_ActionInterface::stripLeadingSlash(const string& s) {
        if (s[0] == '/') return std::move(s.substr(1));
        return std::move(s.substr());
    }

    void place_ActionInterface::prepareRobot() {
    ...
    void place_ActionInterface::lowerHead() {
        ...
        ros::ServiceClient look_around_client = _nh.serviceClient<std_srvs::Empty>("/look_around_interface/tiago_look_around");
        std_srvs::Empty e;
        look_around_client.call(e);
        ROS_INFO("(%s): Done.", node_name.c_str());
    }

    void place_ActionInterface::liftTorso() {
    ...
    bool place_ActionInterface::moveGripperTo(const geometry_msgs::PoseStamped& targetPose) {
    ...
    bool place_ActionInterface::concreteCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) {
        liftTorso();
...
int main(int argc, char** argv) {
    ...
    ros_app::place_ActionInterface rpti;
