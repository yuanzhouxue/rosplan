#include "InhandPredicateObservation.h"
...
namespace rosplane {
    InhandPredicateObervation::InhandPredicateObervation() {
        ...
        service = nh.advertiseService("my_inhand_sensor", &InhandPredicateObervation::callback, this);
    }

    bool InhandPredicateObervation::callback(tiago_despot::SensorSrv::Request &req, tiago_despot::SensorSrv::Response &res) {
        rosplan_dispatch_msgs::SensorDispatchPtr sd = boost::make_shared<rosplan_dispatch_msgs::SensorDispatch>(req.action);
        res.sensor_success = concreteCallback(sd);
        return true;
    }
